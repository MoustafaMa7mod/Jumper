...
public class Trip {
	...
	public Trip(int ID,String tripName, String locationCompany, String companyName,
			Double budget, String programme, Integer duration,
...
public static String addTrip(int companyID, String name , double budget , String programme , 
		  int duration , Date startDate , Date endDate,String transportation, String category) throws SQLException
{
	 ...
	 Connection conn = DBConnection.getActiveConnection();
		...
		PreparedStatement stmt = conn.prepareStatement(sql);
			...
			if(rs.next())
			{
			 ...
			 sql = "select location from company_location where companyID =" +companyID+ " ;";
				...
				if(rs.next())
				{
				 ...
				 sql = "insert into trips "
				 	+ "(companyID , name , companyName , companyLocation ,budget, programme , duration , startDate , endDate , transportation , category )"
				 		+ " values ( "+companyID+ " , '"+name+"' , '"+cName+"' , '"+location+"' , "+budget+ " , '"+programme+"' , "
				 		 +duration+ " , '"+startDate + "'  , '"+endDate +"' ,  '"+transportation +"' , '"+category + "' ) ;" ;
...
public static ArrayList<Trip> tripsForA_company(int companyID , int userID) throws SQLException
{
ArrayList<Trip>trip=new ArrayList<>();
...
public static int updateBadget (int tripID , double newbudget , String password) throws SQLException
{
	Connection con = DBConnection.getActiveConnection();
	...
	sql="select * from company where ID = "+companyID+ " and password = "+ "'"+password+"'" +";" ;
	...
	if(rs.next())
	{
		sql = "Update trips set budget = " + newbudget+ "  where ID = " + tripID + ";" ;
...
public static int updateProgramme (int tripID , String newProgramme , String password) throws SQLException
{
	Connection con = DBConnection.getActiveConnection();
	int companyID = 0;
	String sql="select companyID from trips where ID = "+tripID+ " ;";
	...
	if(rs.next())
	{
		sql = "Update trips set programme = "+ "'" + newProgramme+ "'" + " where ID = " + tripID + ";" ;
...
public static int updateStratTimeandDuration (int tripID , Date newStratTime , int newDuration  , String password) throws SQLException
{
	Connection con = DBConnection.getActiveConnection();
	int companyID = 0;
	String sql="select companyID from trips where ID = "+tripID+ " ;";
	...
	if(rs.next())
	{
		sql = "Update trips set startDate = " +"'" +newStratTime+ "'" + " , duration = " + newDuration + " where ID = " + tripID+";"  ;
...
public static int updateEndTimeandDuration (int tripID , Date newEndTime , int newDuration  , String password) throws SQLException
{
	Connection con = DBConnection.getActiveConnection();
	int companyID = 0;
	String sql="select companyID from trips where ID = "+tripID+ " ;";
	...
	if(rs.next())
	{
		sql = "Update trips set endDate = " +"'" +newEndTime+ "'" + " , duration = " + newDuration + " where ID = " + tripID+";"  ;
...
public static int deleteTrip (int tripID , String password) throws SQLException
{
	Connection con = DBConnection.getActiveConnection();
	int companyID = 0;
...
public static ArrayList<Trip> relatedTrips(int userID , int tripID) throws SQLException
{
	ArrayList<Trip>trip=new ArrayList<>();
	...
	sql = "select * from trips where category = '"+category+"' and ID <> "+tripID+"  order by rate  ;"  ; 
	stmt = con.prepareStatement(sql);
	rs = stmt.executeQuery();
